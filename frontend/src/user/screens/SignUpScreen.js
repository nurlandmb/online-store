...
function SignInScreen(props) {
  ...
  const submitHandler = (e) => {
    ...
    if (phone.replace(/\D/g, '').length !== 11) {
      toast.warn('Номер введен неправильно', {
  ...
  }, [props.profile.isLoggedIn, navigate]);

  return (
    <section className="section sign">
      <div className="container">
        <h2 className="section-title sign__title">Создать аккаунт</h2>
        <div className="sign__wrapper">
          <SignComplete active="active" />
          <form className="sign__form" onSubmit={submitHandler}>
            ...
            <button className="sign__form-submit">Создать</button>
          </form>
        </div>
      </div>
    </section>
